...
- (void)setupFlatUI
{
    ...
    [self.searchButton removeTitleShadow];
    
    /*
     [self.searchButton configureFlatButtonWithColor:[UIColor alizarinColor]
    ...
    [self.searchBar setBackgroundImage:[UIImage imageWithColor:[UIColor peterRiverColor] cornerRadius:0]];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // if someone previously searched, show the search bar
    if (self.searchPredicate)
    {
        self.tableView.tableHeaderView = self.searchBar;
        [self.searchBar becomeFirstResponder];
